...
public final class RowRange
{
     ...
     * An invalid instance.
     */
    public static final RowRange INVALID = new RowRange(MIN_VALID_INDEX - 1, MIN_VALID_INDEX - 1);
     ...
     */
    @NotNull
    public RowRange computeIntersection(final int otherStart, final int otherEnd)
    {
        if (!isValid())
        {
        ...
        {
            return new RowRange(otherStart, otherEnd);
        }
        else if (startIsInRange)
        {
            return new RowRange(otherStart, end);
        }
        else if (endIsInRange)
        {
            return new RowRange(start, otherEnd);
        }
     ...
     */
    @NotNull
    public RowRange computeUnion(final int otherStart, final int otherEnd)
    {
        if (!isValid())
        {
            return RowRange.INVALID;
        ...
        return new RowRange(Math.min(start, otherStart), Math.max(end, otherEnd));
    }
